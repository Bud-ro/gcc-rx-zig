		     ...
		     unsigned int flags ATTRIBUTE_UNUSED) 
{
  ...
  if ( TREE_CODE(TREE_TYPE(decl)) == ARRAY_TYPE )
    {
      sprintf (name, ".rodata.align%d", (int)((TYPE_ALIGN(TREE_TYPE(TREE_TYPE(decl)))) / 8));
    }
  else
    {
      sprintf (name, ".rodata.align%d", (int)((TYPE_ALIGN(TREE_TYPE(decl))) / 8));
    }
...
/* Sort initialised variables based on their alignment. */
section*
		   ...
		   unsigned int flags ATTRIBUTE_UNUSED) 
{
  ...
  if ( TREE_CODE(TREE_TYPE(decl)) == ARRAY_TYPE )
    {
      sprintf (name, ".data.align%d", (int)((TYPE_ALIGN(TREE_TYPE(TREE_TYPE(decl)))) / 8));
    }
  else
    {
      sprintf (name, ".data.align%d", (int)((TYPE_ALIGN(TREE_TYPE(decl))) / 8));
    }
...
categorize_decl_for_section (const_tree decl, int reloc)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
  ...
  else if (VAR_P (decl))
    {
	  ...
	  if (flag_sort_data && !flag_data_sections)   /* Enabled only with -Os */
       {
			    ...
			    unsigned HOST_WIDE_INT align)
{
  ...
  switch (categorize_decl_for_section (decl, reloc))
	...
	case SECCAT_RODATA_ALIGN:
      return align_rodata_section(decl, align, 0);
    case SECCAT_DATA_ALIGN:
      return align_data_section(decl, align, 0);
    case SECCAT_SRODATA:
      sname = ".sdata2";
      break;
    case SECCAT_DATA:
