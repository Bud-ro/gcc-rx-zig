  ...
  for (i = 0; i < loop->num_nodes; i++)
    for (bsi = gsi_start_bb (body[i]); !gsi_end_p (bsi); gsi_next (&bsi))
      if (stmt_can_terminate_bb_p(gsi_stmt(bsi))) {
        /* Relax the "only exit" condition here by treating regular calls as the
           no-throw / no-return-twice builtins are in stmt_can_terminate_bb_p ();
           NOTE: previously it was a bug here that returned 'true' always but
           enabled some optimizations.  */
        if (gimple *t = gsi_stmt(bsi); is_gimple_call(t)) {
          int call_flags = gimple_call_flags(t);
          if ((call_flags & ECF_NOTHROW) && !(call_flags & ECF_RETURNS_TWICE))
            continue;
        }
        return false;
      }
