			     ...
			     bool whole_program)
{
  ...
  /* Keep interrupt routines. */
  if (lookup_attribute ("fast_interrupt", DECL_ATTRIBUTES (node->decl)))
     return true;
  if (lookup_attribute ("interrupt", DECL_ATTRIBUTES (node->decl)))
     return true;
...
varpool_node::externally_visible_p (void)
{
  ...
  /* Variables declared in a named section are considered externally visible. */
