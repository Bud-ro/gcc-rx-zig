...
char *getBuffer(const char *path)
{
  long size;
  char *buffer = 0;
...
final_1 (rtx_insn *first, FILE *file, int seen, int optimize_p)
{
  ...
  if(flag_rxpeephole)
  {
    ...
    do
    {
      if (optimize)
        {		
          static int incr_tmp = 0; 
	  unsigned incr_path = 0;
   	  char incr_buff[5];

          const char * TmpPath = "";
          char *buffer = 0, *buffer1 = 0, *buffer2 = 0, *set = 0, *set1 = 0;

          set = (char *)xmalloc (260);
          memset(set, 0, 260*sizeof(char));
          set1 = (char *)xmalloc (260);
          memset(set, 0, 260 *sizeof(char));
          strncpy(set, exec_path[0], strlen(exec_path[0]) - strlen(strrchr(exec_path[0],'/') + 1));
          strcpy(set1, set);
	  ...
	  sprintf(incr_buff, "%d",incr_tmp);
	  char *path = (char *)xmalloc (260); 
	  #ifdef _WIN32
            strcpy(path, getenv("TMP"));
            strcat(path, "\\rx.dump");
            if(TmpPath == 0)
            {
              break;
          ...
          #else
            strcpy(path, "/tmp/rx.dump");
          #endif
	  
	  strcat (path, incr_buff);
          TmpPath = path;

	  incr_tmp ++;
          FILE *tempFile = fopen(TmpPath, "wb");
          if(tempFile == NULL)
          {
            break;
          }
          dump_rtl_slim (tempFile, get_insns(), NULL, -1, 0);
          fclose(tempFile);

          buffer = getBuffer(TmpPath);
          if (buffer == NULL)
          {
            break;
          }

          char CheckPath[275], ReplacePath[275];

	  while(1)
	  {
	    sprintf(CheckPath, "%scheck/%d.txt",set, incr_path);
	    sprintf(ReplacePath, "%sreplace/%d.txt",set1,incr_path);
  
	    buffer1 = getBuffer(CheckPath);
            if (buffer1 == NULL)
            {
              break;
            }

	    buffer2 = getBuffer(ReplacePath);
            if (buffer2 == NULL)
            {
              break;
            }

            const char *core_bench_state = buffer1;
            if(strlen(buffer) == strlen(core_bench_state))
            {
            if(strcmp(buffer, core_bench_state) == 0)
              {
		generate_insn++;
                const char * core_bench_state_asm = buffer2;
                fprintf (asm_out_file, "%s", core_bench_state_asm);
              }
	    }
	  incr_path++;
  	  }
  ...
  /* Output the insns.  */
  if(!flag_rxpeephole || (flag_rxpeephole && !generate_insn))
	{
  for (insn = first; insn;)
    {
      if (HAVE_ATTR_length)
