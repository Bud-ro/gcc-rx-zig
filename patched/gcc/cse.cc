...
#include "config.h"
/* <memory> and <functional> are needed for the 'iteration_count' hack in fold_rtx () */
...
fold_rtx (rtx x, rtx_insn *insn)
{
  ...
  /* Limit the number of iterations (or actually the depth of recursion). */
  static unsigned int iteration_count = 0;

  std::unique_ptr<void, std::function<void(void*)>>
	dec_iteration_count_on_ret((void*)1, [&](void*) { --iteration_count; });
  ...
  for (path_entry = 0; path_entry < path_size; path_entry++)
      ...
      FOR_BB_INSNS (bb, insn)
	{
	  ...
	  if (NONDEBUG_INSN_P (insn)
		...
		if (INSN_P (insn) && GET_CODE(PATTERN(insn)) == UNSPEC_VOLATILE)
			{
				flush_hash_table ();
				num_insns = 0;
      }
