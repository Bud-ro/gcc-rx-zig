		 ...
		 tree last_decl /* = NULL_TREE */)
{
  ...
  if (TREE_CODE (*node) == FUNCTION_DECL
    ...
    /* An "interrupt" or "fast_interrupt" function attribute implies "no_icf"
     for those targets that support it.  */
  if (TREE_CODE (*node) == FUNCTION_DECL
      && attributes
      && (lookup_attribute ("interrupt", attributes) != NULL || lookup_attribute ("fast_interrupt", attributes) != NULL)
      && (lookup_attribute_spec (get_identifier ("interrupt")) || lookup_attribute_spec (get_identifier ("fast_interrupt"))))
    {
